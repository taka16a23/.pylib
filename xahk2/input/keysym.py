...
for key, value in KEYSYMS.items():
    KEYSYMS[value] = key
...
class KeySym(object):
    ...
    def from_name(cls, name):
        ...
        @Error:
        """
        return cls(KEYSYMS.get(name, 0), name)
    ...
    def from_number(cls, number):
        ...
        @Error:
        """
        return cls(number, KEYSYMS.get(number, ''))
    ...
    def is_special(self, ):
        ...
        @Error:
        """
        return self.number in (0, 0x00ffffff)
    ...
    def is_latin1(self, ):
        ...
        @Error:
        """
        return (0x0020 <= self.number <= 0x007e or 0x00a0 <= self.number <= 0x00ff)
    ...
    def is_unicode(self, ):
        ...
        @Error:
        """
        return (0x01000100 <= self.number <= 0x0110ffff)
    ...
    def to_char(self, ):
        ...
        """
        if self.is_special():
            # TODO: (Atami) [2015/12/07]
            raise StandardError()
        if self.is_latin1():
            return unichr(self.number)
        if self.is_unicode():
            return unichr(self.number - 0x01000000)
        if self.is_legacy():
            return unichr(LEGACY[self.number])
        # TODO: (Atami) [2015/12/07]
        raise StandardError()

    def __repr__(self):
        values = '{0.number}, "{0.name}"'.format(self)
        if self.is_nosymbol():
            values = 'NoSymbol'
        return '{0.__class__.__name__}({1})'.format(self, values)

    def __eq__(self, other):
        if not isinstance(other, (KeySym, )):
            return False
        return (self.number, self.name) == (other.get_number(), other.get_name())
