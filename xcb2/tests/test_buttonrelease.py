...
# $Id: test_buttonrelease.py 342 2015-07-24 05:07:32Z t1 $
# $Revision: 342 $
...
class TestButtonReleaseBinary(MockerTestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_binary1(self):
        r"""Test SendEvent.ButtonRelease binary1."""
        binary = self.protocol.ButtonRelease._getbinary(*self.args1[0])
...
class TestSendEventRequest(MockerTestCase):
    ...
    def test_call(self):
        r"""Test SendEvent.ButtonRelease.__call__() expect return VoidCookie."""
        cookie = self.protocol.ButtonRelease(*self.argsbin[0])
        self.assertIsInstance(cookie, VoidCookie,
                              msg='Failed: {0}({1}) not returned'
                              ' VoidCookie\ngot: {2}'
                              .format(self.protocol.ButtonRelease
                                      .__class__.__name__,
                                      str(self.argsbin[0])
                                      .replace('(', '').replace(')', ''),
                                      cookie))
...
# coding: utf-8
# End:
# test_buttonrelease.py ends here
