...
# $Id: test_motionnotify.py 342 2015-07-24 05:07:32Z t1 $
# $Revision: 342 $
...
class TestMotionNotifyBinary(MockerTestCase):
    @classmethod
    def setUpClass(cls):
        cls.args1 = [(False, 44040214,
                      ...
                      '\x00\x00\x00\x01\x00']
        cls.args2 = [(False, 44040214, EventMask.Button1Motion,
                       1, 0, 0, 482, 44040214, 0, 0, 0, 0, 0, 0, 1),
                      '\x00\x00\x00\x00\x16\x00\xa0\x02\x00\x01\x00\x00\x06'
                      '\x01\x00\x00\x00\x00\x00\x00\xe2\x01\x00\x00\x16\x00'
                      '\xa0\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
    ...
    def test_binary2(self):
        r"""Test SendEvent.MotionNotify binary2."""
        binary = self.protocol.MotionNotify._getbinary(
            *self.args2[0])
        self.assertEqual(self.args2[1], binary,
                         msg='Failed {0.__class__.__name__}.{1.__name__}({2}):\n'
                         'Expected: "{3}"\nReturned: "{4}"'
                         .format(self.protocol, self.protocol._getbinary,
                                 str(self.args2[0])
                                 .replace('(', '').replace(')', ''),
                                 repr(self.args2[1]), repr(binary)))
...
class TestMotionNotifyRequest(MockerTestCase):
    @classmethod
    def setUpClass(cls):
        cls.argsbin = [(False, 44040214, EventMask.Button1Motion,
                       1, 0, 0, 482, 44040214, 0, 0, 0, 0, 0, 0, 1),
                      '\x00\x00\x00\x00\x16\x00\xa0\x02\x00\x01\x00\x00\x06'
                      '\x01\x00\x00\x00\x00\x00\x00\xe2\x01\x00\x00\x16\x00'
                      '\xa0\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
    ...
    def test_call(self):
        r"""Test SendEvent.MotionNotify.__call__() expect return VoidCookie."""
        cookie = self.protocol.MotionNotify(*self.argsbin[0])
...
# coding: utf-8
# End:
# test_motionnotify.py ends here
